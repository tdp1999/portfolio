...
import { Card, Eyebrow, Icon, Background, Link, StatusDot } from '@portfolio/landing/shared/ui';

import { DdlDecisionRecord } from '../ddl-decision-record/ddl-decision-record';
import { DdlDocPage } from '../ddl-doc-page/ddl-doc-page';
import { DdlSection } from '../ddl-section/ddl-section';
import type { CopyState, BleedId, HoursId } from './ddl-bio.types';
import { BIO_VARIANTS, HOURS_FROM, HOURS_TO } from './ddl-bio.data';

/**
 * /ddl/bio — refinement record for the home §3 Bio Card Grid pass.
 *
 * Six independent micro-decisions, each with its own pick:
 ...
 *
 * The decision record (BIO_VARIANTS) lists all six settled picks; each section
 * below keeps the full demo so the alternatives stay comparable.
 */

@Component({
  selector: 'landing-ddl-bio',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [Card, Eyebrow, Icon, Background, Link, StatusDot, DdlDocPage, DdlSection, DdlDecisionRecord],
  templateUrl: './ddl-bio.html',
  styleUrl: './ddl-bio.scss',
})
export class DdlBio {
  readonly variants = BIO_VARIANTS;
