...
@CommandHandler(MarkAsUnreadCommand)
export class MarkAsUnreadHandler implements ICommandHandler<MarkAsUnreadCommand> {
  constructor(@Inject(CONTACT_MESSAGE_REPOSITORY) private readonly repo: IContactMessageRepository) {}

  async execute(command: MarkAsUnreadCommand): Promise<void> {
    ...
    const updated = message.markAsUnread();
