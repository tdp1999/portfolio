...
export class SoftDeleteMessageCommand {
  constructor(readonly messageId: string) {}
}

@CommandHandler(SoftDeleteMessageCommand)
export class SoftDeleteMessageHandler implements ICommandHandler<SoftDeleteMessageCommand> {
  constructor(@Inject(CONTACT_MESSAGE_REPOSITORY) private readonly repo: IContactMessageRepository) {}

  async execute(command: SoftDeleteMessageCommand): Promise<void> {
    ...
    const updated = message.softDelete();
