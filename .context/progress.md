...
- [~] 361-content-authoring-master - **Tracker duy nhất cho content** (EN + VI, mọi surface). Brief còn mở bên dưới: 328 (`/now`), 323 (`llms.txt`); 340/341 đã đóng vì trùng (2026-07-27), static-UI VI thuộc 388 (**đóng 2026-07-28**, cả 4 batch dịch xong). Đang author project Document Engine (tab Basic/Story/Highlights xong, tới Media). **Đối chiếu lại với 388/397 ngày 2026-07-28:** content nay có **hai nhà** — Console (prod) và dictionary trong codebase (deploy để publish), nên thêm mark `[dict]` = có chữ EN+VI nhưng chữ máy viết, chưa duyệt giọng; `/uses`, `/colophon`, legal, microcopy `/contact` chuyển từ "trống" sang `[dict]`, và mục nợ lớn nhất giờ là **duyệt giọng 476 key dictionary** (không hiện ở bảng gap vì trên prod nó không trống) (XL) (standalone, in-progress)
...
- [~] 396-ddl-motion-catalogue-and-capture-skill - Hoàn thiện trang `/ddl/motion` (catalogue motion landing) + đóng gói/validate skill "video-from-capture"; scaffold + 4 clip pilot đã có, clip đang mờ/lỗi, còn 8 clip. Là B2 trong ledger document-engine (L) (standalone, in-progress)
...
_**(1) Trùng số task.** Có hai task 390 và hai task 391. Lệnh dò số kế tiếp trong `/ctx:create-task` + `/ctx:breakdown` dùng `ls .context/tasks-done/` phẳng, mà thư mục đó lồng theo epic (`tasks-done/epic-design-skill/…`), nên nó trả về **tên thư mục** chứ không phải task: mọi task đã archive đều vô hình, số vừa archive xong được cấp lại ngay. Hai file mới hơn đã đánh số lại: `390-ddl-motion → 396`, `391-landing-i18n-post-review-hardening → 397`. Hai file archive giữ nguyên số vì chúng đã nằm trong lịch sử git. Commit `35feacd6` vẫn ghi "388+391" — số cũ, không sửa được. Cặp `065` trùng nhau (`065-optimize-landing-serve-performance` + `065-restructure-libs-architecture`) là di sản cũ hơn, cả hai đã archive, để nguyên._

_**(2) Task 381 báo `done` mà không AC nào được tick.** Dòng status là `## Status: done (pending visual verify after console dev-server restart)`. Cái ngoặc đơn đó chính là bằng chứng task chưa xong, nhưng nó cũng làm dòng status không khớp token nào: `/ctx:sync` không archive mà cũng không báo lỗi, task lặng lẽ rơi khỏi mọi con số. Đã đổi về `in-progress` (3/3 AC vẫn trống, chờ restart console dev server để bỏ bundle document-engine 0.1.2 cũ rồi nhìn dropdown thật). **Cùng ngày owner xác nhận đã nhìn thấy dropdown đúng** → tick cả 3 AC, status `done`, archive về `tasks-done/epic-portfolio-rich-text-editor/`. Đường đi đúng là như vậy: người kiểm chứng tick AC, rồi mới tới `done` — chứ không phải ghi `done` kèm lời hứa sẽ kiểm chứng sau._

_Nguyên nhân gốc đã vá ở **plugin ctx 2.2.0** (repo `claude-code-ctx`): lệnh dò số chuyển sang `find … | sort -n` đệ quy + kiểm tra số còn trống trước khi ghi; `/ctx:sync` thêm bước integrity check (quét số trùng, quét mâu thuẫn status/AC) và chỉ archive khi status đúng token `done` **và** mọi AC đã tick; `/ctx:task` chặn ghi `done` khi còn AC trống và cấm mọi hậu tố trên dòng status._
