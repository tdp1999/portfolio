...
### Typography
- [ ] Body text is at least 16px (no text below 14px except captions/badges)
- [ ] Line-height is 1.5 for body text, 1.25 for headings (2xl+)
- [ ] Prose text blocks have `max-width: 65ch` or equivalent (~45-75 chars/line)
- [ ] Heading scale follows ~1.2 Minor Third ratio (or close)
- [ ] Letter-spacing: tighter for display headings, normal for body

### Spacing
- [ ] All spacing uses 8px grid multiples (4, 8, 12, 16, 24, 32, 48, 64, 96)
- [ ] Inner spacing < outer spacing for grouped elements (Gestalt proximity, 1:2+ ratio)
- [ ] Section spacing is py-16 (mobile) / py-24 (desktop)

### Color & Contrast
- [ ] All text meets WCAG AA contrast (4.5:1 normal text, 3:1 large text)
- [ ] Non-text UI elements (borders, icons, form controls) meet 3:1 contrast
- [ ] Primary text on dark surfaces uses #e2e8f0 (~87% white), not pure #fff
- [ ] Verify both light and dark modes

### Interactive Targets
- [ ] All buttons are at least 40px height (desktop)
- [ ] All clickable/tappable elements have at least 44px touch target (for mobile)
- [ ] Minimum 8px gap between adjacent interactive elements

### Component Sizing
- [ ] Buttons: sm=32px, md=40px, lg=48px
- [ ] Inputs: 40px height
- [ ] Icons: consistent sizing (16/20/24/32px per usage context)
- [ ] Border radius: consistent per component type (lg for buttons/inputs, xl for cards)

### Layout
- [ ] Content containers: max-w-4xl (standard), max-w-6xl (wide grids)
