...
## Acceptance Criteria
- [ ] Profile settings page (Identity section): avatar field renders current avatar preview + "Change" button → opens MediaPickerDialog (single, `mimeFilter: 'image/'`, default folder `avatars`).
- [ ] Profile settings page (SEO section): OG image field same pattern, default folder `logos`.
- [ ] Selecting a media in picker sets the corresponding form control to the Media ID.
- [ ] "Remove" button clears the field (sets null).
- [ ] Save persists the new avatar/og-image IDs via existing PATCH endpoints.
- [ ] Inline upload UI and its handlers deleted from profile page.
- [ ] Preview updates immediately after selection (uses Media URL from picker result).
- [ ] Existing profile entity/commands unchanged (still accept avatarId / ogImageId).
- [ ] Visual check in browser: avatar + og image flows work end-to-end.
