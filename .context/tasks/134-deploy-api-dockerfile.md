...
## Acceptance Criteria
- [ ] `apps/api/Dockerfile` created with multi-stage build
- [ ] Stage 1 (builder): `node:20-alpine`, installs pnpm, copies monorepo, runs `nx build api --configuration=production`, generates Prisma client
- [ ] Stage 2 (production): `node:20-alpine`, copies only `dist/apps/api/`, Prisma schema + migrations, generated Prisma client
- [ ] `CMD` runs `prisma migrate deploy` before starting the app (`sh -c "npx prisma migrate deploy --schema=./prisma/schema.prisma && node main.js"`)
- [ ] `.dockerignore` created/updated to exclude `node_modules`, `.git`, `dist`, test files, IDE configs
- [ ] Image builds successfully: `docker build -f apps/api/Dockerfile -t portfolio-api .`
- [ ] Container starts and responds to `/api/health` (with database connection)
- [ ] Image size is under 300MB
- [ ] `EXPOSE 3000` included
