...
- **Cursor** — the stage cursor stays default; it only hints `zoom-in`/`zoom-out` while ⌘/Ctrl is held. The trigger thumbnail uses `pointer`. A desktop-only hint (`@media (hover) and (pointer: fine)`) shows the zoom shortcuts and hides once zoomed.
