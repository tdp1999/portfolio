...
BANK_DIR="$CLAUDE_PROJECT_DIR/.context/design"
...
# Check for a matching per-component doc
PATTERN_FILE="$BANK_DIR/components/$COMPONENT.md"

if [ -f "$PATTERN_FILE" ]; then
  echo "{\"additionalContext\": \"Design bank has guidelines for '$COMPONENT'. Read .context/design/components/$COMPONENT.md before making this edit.\"}"
else
  # Check if bank has any entries at all
  if [ -f "$BANK_DIR/index.md" ]; then
    echo "{\"additionalContext\": \"Design bank is available at .context/design/index.md — check for relevant UI guidelines before editing.\"}"
  fi
fi
