...
| **Agent model = ask before spawning** | Before spawning any agent (Agent/Task tool, incl. `fork`), check its resolved model. If it is **Opus or higher (Fable)**, you MUST ask the user for approval first — do NOT spawn without it. **Haiku or Sonnet** need no approval. When unsure of the resolved model, ask. | ❌ spawn Opus fork silently ✅ "This agent runs Opus — ok to spawn?"; ✅ spawn a Sonnet Explore agent directly |
